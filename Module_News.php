...
//			return ''; // No edit, quit silently?
	...
	public function sidebarContent($bar)
	{
		if ($bar === 'left')
		{
			return $this->newsSidebar();
		}
	}
	
	private function newsSidebar()
	{
		$this->onInclude();
		$this->onLoadLanguage();
		$user = GWF_Session::getUser();
		$tVars = array(
			'news' => GWF_News::getNews(5, 0, 1, 'news_date DESC', false),
			'href_add' => GWF_WEB_ROOT.'news/add',
			'can_add' => GWF_User::isAdminS(),
			'can_sign' => $this->canSignNewsletter($user),
		);
		return $this->template('news-sidebar.php', $tVars);
	}
}
